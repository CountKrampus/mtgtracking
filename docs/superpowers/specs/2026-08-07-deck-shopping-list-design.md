...
1. Build `ownedScryfallIds`/`ownedNames` sets from `cards` (the collection): `ownedScryfallIds` from every collection card that has one, `ownedNames` (case-insensitive) from *every* collection card regardless of whether it has a `scryfallId` — broader than `decks.js`'s `/recommendations` route (which only falls back to name for collection cards missing a `scryfallId`), because here a *deck* card can itself lack a `scryfallId` while the same card is owned under a `scryfallId`'d printing, so name still needs to be checked against the full collection, not just the un-IDed subset of it.
